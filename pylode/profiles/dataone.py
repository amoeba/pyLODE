...
class DataONE(BaseProfile):
    ...
    def _load_template(self, template_file):
        return Environment(loader=FileSystemLoader(join(TEMPLATES_DIR, "dataone"))).get_template(template_file)
    ...
    def _make_fid(self, title, uri):
        """Custom override of _make_fid behavior so the fragment is the full
        term URI instead of its label. Makes it possible to link people to
        portions of the page by URI"""
        return super()._make_fid(uri, uri)
